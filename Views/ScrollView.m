...
//    self.backgroundColor = [UIColor whiteColor];
    ...
    _indicatorView.backgroundColor = UICOLORRGB(0x3ea0e6);
...
-(void)createBtn{
    ...
    for (int i=0; i<_headArray.count; i++) {
        ...
        button.frame = CGRectMake(BtnWidth * i, 0, BtnWidth, 45);
        button.titleLabel.font = [UIFont systemFontOfSize:17];
        [button setTitle:_headArray[i] forState:UIControlStateNormal];
        [button setTitleColor:UICOLORRGB(0x282828) forState:UIControlStateNormal];
        [button setBackgroundColor:[UIColor whiteColor]];
        if (i==0) {
            [button setTitleColor:UICOLORRGB(0x3ea0e6) forState:UIControlStateNormal];
...
-(void)changeBtntitleColorWith:(NSInteger)tag{
    ...
    [self.subviews enumerateObjectsUsingBlock:^(__kindof UIButton * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([obj isKindOfClass:[UIButton class]]) {//判断该subViews是否是button
            ...
            if (obj.tag == tag) {//subViews的tag与按钮的tag一样
                //改变颜色
                [obj setTitleColor:UICOLORRGB(0x3ea0e6) forState:UIControlStateNormal];
                ...
                [obj setTitleColor:UICOLORRGB(0x282828) forState:UIControlStateNormal];
